    ...
    public static String getDefaultActiveProfiles(){
        if (BUILD_PROPERTIES != null) {
            ...
            if (activeProfile != null && !activeProfile.isEmpty() &&
                (activeProfile.contains(AppConstant.SPRING_PROFILE_DEVELOPMENT) ||
                    activeProfile.contains(AppConstant.SPRING_PROFILE_PRODUCTION))) {
                return activeProfile;
            }
        }
        log.warn("No Spring profile configured, running with default profile: {}", AppConstant.SPRING_PROFILE_LOCAL);
        return AppConstant.SPRING_PROFILE_LOCAL;
    }
